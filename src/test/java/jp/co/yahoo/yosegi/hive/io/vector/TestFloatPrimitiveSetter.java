...
public class TestFloatPrimitiveSetter{

  @Test
  public void T_set_1() throws IOException{
    IColumn column = new PrimitiveColumn( ColumnType.FLOAT , "t" );
    ...
    for( int i = 0 ; i < 3 ; i++ ){
      ...
      for( int n = 0 ; n < 1024 ; n++ ){
        if( ( n + start ) < 2000 ){
          assertEquals( vector.vector[n] , Float.valueOf( (float)( n + start ) / (float)1000 ).doubleValue() );
        }
        else{
